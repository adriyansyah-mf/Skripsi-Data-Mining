...
import streamlit_extras as stext
...
selected = option_menu(
            menu_title=None,
            options=["Visualisasi","Update Model","Prediksi"],
            icons=["bar-chart-line-fill","arrow-up-circle-fill","diagram-3-fill"],
            default_index=0,
...
if selected == "Update Model":
    ...
    if data_baru is not None:
        ...
        if st.button('Update Model'):
            ...
            pickle_1.dump(modelrf, open(filename,"wb"))
            st.balloons()
...
if selected == "Prediksi":
    ...
    with st.expander("Input CSV"):
        ...
        if newfile is not None:
            ...
            file = dropna(file)
            file_clean = clean_data_input(file)

            st.subheader("Dataframe Dari File CSV")
            st.dataframe(file)        
    ...
    with st.expander("Input Manual"):
        ...
        with st.form('Form 1'):
            ...
            if submitted is True :
                ...
                load_model = pickle_1.load(open("modelrf_final.sav","rb"))                   
                ypredict = load_model.predict_proba(newdata)
                pred = round(ypredict[0][1],5)
                                                        
                st.subheader('Prediksi Karyawan Tersebut Resign Adalah')
                st.text(ypredict)
                percent="%"
                result=str(pred*100) + percent 
